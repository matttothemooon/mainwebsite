...
const DEFAULT_GEAR = [
  { name: "Gaming PC", items: [{ label: "Example", value: "Add a product in the admin panel", url: "" }] },
...
export function validateProfile(raw) {
  ...
  return {
    ...
    changelog: Array.isArray(raw.changelog)
      ? raw.changelog.slice(0, 100).map(cleanChangelog).filter((entry) => entry.id && entry.title && entry.body)
      : DEFAULT_CHANGELOG,
  };
}
